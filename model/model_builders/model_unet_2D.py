...
from common_utils.regularizers import *
from model.loss.dice_loss import *
from math import ceil
from model.networks.neural_network import *


class UNet_2D(ModelBase):

    def __init__(self, opt):
        super(UNet_2D, self).__init__(opt)
        # ------------------------------------
        # define network
        # ------------------------------------
        self.image = None
        ...
        opt_net = opt['netSeg']
        self.C = opt_net['out_chans']
        # only for Conv UNet
        self.input_shape_must_be_divisible_by = opt['netSeg']['shape_must_be_divisible_by']
    ...
    def get_total_loss(self):
        ...
        if self.gt_seg.shape[1] == 1:
            self.loss_ce = self.celoss(self.pred_seg, torch.squeeze(self.gt_seg, 1).long())
        else:
            # for braTS acc bceloss for each class
            bces = []
            for i in range(1, C):
                loss_bce = self.bceloss(self.sigmoid(self.pred_seg[:, i, :, :]), torch.squeeze(self.gt_seg[:, i, :, :]).float())
                bces.append(loss_bce)
            self.loss_ce = sum(bces)
            self.loss_ce /= (C-1)
    ...
    def feed_slice_from_4D(self, idx_slice):
        self.image = self.images[:, idx_slice, :, :, :]
        if len(self.gt_segs.shape) == 4:
            self.gt_seg = self.gt_segs[:, idx_slice:(idx_slice + 1), :, :]
        else:
            self.gt_seg = self.gt_segs[:, idx_slice, :, :, :]
    ...
    def netSeg_predict_2D_tiled(self, patch_size):
        self.pred_seg, _ = self.netSeg.module.predict_2D(self.image[0].detach().cpu().numpy(), do_mirroring=True, patch_size=patch_size, verbose=False)
        self.pred_seg = torch.tensor(self.pred_seg).to(self.device)
    ...
    def optimize_parameters(self, current_step):
        ...
        if G_lossfn_type == 'ce':
            self.log_dict['loss_ce'] = self.loss_ce.item()
        if G_lossfn_type == 'dice':
    ...
    def optimize_parameters_accum(self, current_step, accumulation=1):
        self.netSeg_forward()
        total_loss = self.get_total_loss()
        total_loss.backward()
        ...
        # ------------------------------------
        # `clip_grad_norm` helps prevent the exploding gradient problem.
        Seg_optimizer_clipgrad = self.opt_train['Seg_optimizer_clipgrad'] if self.opt_train['Seg_optimizer_clipgrad'] else 0
        if Seg_optimizer_clipgrad > 0:
            torch.nn.utils.clip_grad_norm_(self.parameters(), max_norm=self.opt_train['Seg_optimizer_clipgrad'], norm_type=2)

        if (current_step+1)%accumulation == 0:
            self.Seg_optimizer.step()
            self.Seg_optimizer.zero_grad()
        ...
        # regularizer
        # ------------------------------------
        Seg_regularizer_orthstep = self.opt_train['Seg_regularizer_orthstep'] if self.opt_train['Seg_regularizer_orthstep'] else 0
        if Seg_regularizer_orthstep > 0 and current_step % Seg_regularizer_orthstep == 0 and current_step % self.opt['train']['checkpoint_save'] != 0:
            self.netSeg.apply(regularizer_orth)
        Seg_regularizer_clipstep = self.opt_train['Seg_regularizer_clipstep'] if self.opt_train['Seg_regularizer_clipstep'] else 0
        if Seg_regularizer_clipstep > 0 and current_step % Seg_regularizer_clipstep == 0 and current_step % self.opt['train']['checkpoint_save'] != 0:
            self.netSeg.apply(regularizer_clip)
    ...
    # ----------------------------------------
    def current_visuals(self, use_tile=False, use_bce = False):
        if use_tile:
            out_dict = OrderedDict()
            H, W = self.pred_seg.shape
            self.gt_seg = mask_to_onehot(torch.zeros(1, self.C, H, W).to(self.device), self.gt_seg)
        ...
        out_dict['gt_seg'] = self.gt_seg.detach()[0].float().cpu().numpy()
        # pred_shape: 1, 4, 224, 224
        if use_bce:
            pred_seg = []
            for i in range(self.pred_seg.shape[1]):
                pred_seg_channel = torch.sigmoid(self.pred_seg[:, i, :, :])
                pred_seg.append(pred_seg_channel.detach()[0].float().cpu().numpy())
            pred_seg = np.array(pred_seg)
            out_dict['pred_seg'] = pred_seg
        else:
    ...
    def collect_for_3d_volumn(self, use_tile=False, use_bce=False):
        output = self.current_visuals(use_tile, use_bce)
