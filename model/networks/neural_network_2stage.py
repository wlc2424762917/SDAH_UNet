...
from common_utils.random_stuff import no_op
from common_utils.to_torch import to_cuda, maybe_to_torch
...
class SegmentationNetwork_2stage(NeuralNetwork):
    def __init__(self):
        super(NeuralNetwork, self).__init__()
    ...
    def predict_2D(self, x, do_mirroring: bool, mirror_axes: tuple = (0, 1), use_sliding_window: bool = True,
                   step_size: float = 0.5, patch_size: tuple = None, regions_class_order: tuple = None,
                   use_gaussian: bool = False, pad_border_mode: str = "constant",
                   pad_kwargs: dict = None, all_in_gpu: bool = False,
                   verbose: bool = False, mixed_precision: bool = True) -> Tuple[np.ndarray, np.ndarray]:
        """
    ...
    def _internal_predict_2D_2Dconv_tiled(self, x: np.ndarray, step_size: float, do_mirroring: bool, mirror_axes: tuple,
        ...
        # adding two results
        aggregated_detection_results = aggregated_results
        aggregated_segmentation_results = aggregated_results
        ...
        for x in steps[0]:
            ...
            for y in steps[1]:
                ...
                predicted_patchs = self._internal_maybe_mirror_and_pred_2D(
                    data[None, :, lb_x:ub_x, lb_y:ub_y], mirror_axes, do_mirroring,
                    gaussian_importance_map)[0]
                predicted_detection_patch = predicted_patchs[0]
                predicted_segmentation_patch = predicted_patchs[1]

                if all_in_gpu:
                    predicted_detection_patch = predicted_detection_patch.half()
                    predicted_segmentation_patch = predicted_segmentation_patch.half()
                else:
                    predicted_detection_patch = predicted_detection_patch.cpu().numpy()
                    predicted_segmentation_patch = predicted_segmentation_patch.cpu().numpy()

                aggregated_detection_results[:, lb_x:ub_x, lb_y:ub_y] += predicted_detection_patch
                aggregated_segmentation_results[:, lb_x:ub_x, lb_y:ub_y] += predicted_segmentation_patch
        ...
        slicer = tuple(
            [slice(0, aggregated_results.shape[i]) for i in
             range(len(aggregated_results.shape) - (len(slicer) - 1))] + slicer[1:])
        aggregated_detection_results = aggregated_detection_results[slicer]
        aggregated_segmentation_results = aggregated_segmentation_results[slicer]

        aggregated_nb_of_predictions = aggregated_nb_of_predictions[slicer]

        # computing the class_probabilities by dividing the aggregated result with result_numsamples
        detection_class_probabilities = aggregated_detection_results / aggregated_nb_of_predictions
        segmentation_class_probabilities = aggregated_segmentation_results / aggregated_nb_of_predictions

        if regions_class_order is None:
            predicted_detection = detection_class_probabilities.argmax(0)
            predicted_segmentation = segmentation_class_probabilities.argmax(0)
        else:
            if all_in_gpu:
                detection_class_probabilities_here = detection_class_probabilities.detach().cpu().numpy()
                segmentation_class_probabilities_here = segmentation_class_probabilities.detach().cpu().numpy()
            else:
                detection_class_probabilities_here = detection_class_probabilities
                segmentation_class_probabilities_here = segmentation_class_probabilities
            predicted_detection = np.zeros(detection_class_probabilities_here.shape[1:], dtype=np.float32)
            predicted_segmentation = np.zeros(segmentation_class_probabilities_here.shape[1:], dtype=np.float32)
            for i, c in enumerate(regions_class_order):
                predicted_detection[detection_class_probabilities_here[i] > 0.5] = c
                predicted_segmentation[segmentation_class_probabilities_here[i] > 0.5] = c
        ...
        if all_in_gpu:
            ...
            if regions_class_order is None:
                predicted_detection = predicted_detection.detach().cpu().numpy()
                predicted_segmentation = predicted_segmentation.detach().cpu().numpy()

            detection_class_probabilities = detection_class_probabilities.detach().cpu().numpy()
            segmentation_class_probabilities = segmentation_class_probabilities.detach().cpu().numpy()

        if verbose:
            print("prediction done")
        return predicted_segmentation, segmentation_class_probabilities, predicted_detection, detection_class_probabilities
...
if __name__ == '__main__':
    print(SegmentationNetwork_2stage._compute_steps_for_sliding_window((30, 224, 224), (162, 529, 529), 0.5))
    print(SegmentationNetwork_2stage._compute_steps_for_sliding_window((30, 224, 224), (162, 529, 529), 1))
    print(SegmentationNetwork_2stage._compute_steps_for_sliding_window((30, 224, 224), (30, 224, 224), 0.125))
    print(SegmentationNetwork_2stage._compute_steps_for_sliding_window((123, 54, 123), (246, 162, 369), 0.25))
