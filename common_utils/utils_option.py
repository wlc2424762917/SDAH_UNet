...
def parse(opt_path, is_train=True):
    ...
    if is_train:
        ...
        print("save_path:", opt['path']['models'])
    else:  # test
    ...
    # network
    # ----------------------------------------
    opt['netSeg']['scale'] = opt['scale'] if 'scale' in opt else 1
    ...
    # ----------------------------------------
    if 'Seg_optimizer_reuse' not in opt['train']:
        opt['train']['Seg_optimizer_reuse'] = False

    # ----------------------------------------
    # default setting of strict for model loading
    # ----------------------------------------
    if 'Seg_param_strict' not in opt['train']:
        opt['train']['Seg_param_strict'] = True

    return opt


def find_last_checkpoint(save_dir, net_type='Seg'):
    """
    Args: 
        save_dir: model folder
        net_type: 'Seg' or 'optimizerSeg'
