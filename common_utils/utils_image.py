...
def augment_img_2D(img, mode=0):
    if mode == 0:
        return img
    elif mode == 1:
        return np.flipud(rotate(img, 90, reshape=False))
    elif mode == 2:
        return np.flipud(img)
    elif mode == 3:
        return rotate(img, 270, reshape=False)
    elif mode == 4:
        return np.flipud(rotate(img, 270, reshape=False))
    elif mode == 5:
        return rotate(img, 90, reshape=False)
...
'''
# --------------------------------------------
# random crop and choose high quality samples
# shape: (H, W, D)
# --------------------------------------------
'''
def randcrop(img_in, shape):
    img = np.copy(img_in)
    if img.ndim == 2:
        H, W = img.shape
        new_H = shape[0] if shape[0] < H else H-1
        new_W = shape[1] if shape[1] < W else W-1
        y = np.random.randint(0, H - new_H)
        x = np.random.randint(0, W - new_W)
        img_out = img[y:y+new_H, x:x+new_W]

    if img.ndim == 3:
        H, W, D = img.shape
        new_H = shape[0] if shape[0] < H else H-1
        new_W = shape[1] if shape[1] < W else W-1
        new_D = shape[2] if shape[2] < D else D-1
        y = np.random.randint(0, H - new_H)
        x = np.random.randint(0, W - new_W)
        z = np.random.randint(0, D - new_D)
        img_out = img[y:y+new_W, x:x+new_H, z:z+new_D]
    return img_out
...
root_path_background = r"C:/Users/wlc/Documents/GitHub/MRI_Recon/mri_segmentation/test_512_new/GTImage/"
root_path_gt = r"C:/Users/wlc/Documents/GitHub/MRI_Recon/mri_segmentation/test_512_new/GTMaskMultiOrganNP/"
output_path_gt_img = r"C:/Users/wlc/Documents/GitHub/MRI_Recon/mri_segmentation/test_512_new/GTMask+GTImage_MultiOrgan/"

root_path_pred = r"C:/Users/wlc/Documents/GitHub/MRI_Recon/mri_segmentation/test_512_new/PredMaskMultiOrganNP/"
output_path_pred_img = r"C:/Users/wlc/Documents/GitHub/MRI_Recon/mri_segmentation/test_512_new/PredMask+GTImage_MultiOrgan/"
