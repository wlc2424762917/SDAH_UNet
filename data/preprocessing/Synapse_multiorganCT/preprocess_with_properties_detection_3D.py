...
def read_img(src_file_path, src_file_name, src_save_path, gt_file_path, gt_file_name, gt_save_path, save_properties_path_pkl_raw, dtype=sitk.sitkFloat32):  # for .mhd .nii .nrrd
    ...
    for slice_idx in range(0, mask_data_norm.shape[0]):
        ...
        classes = [5, 1, 8, 6, 3, 4, 7, 2]  # 按照从难到易, 从大到小
        for c in classes:
        ...
        detection_mask.append(detection_mask_slice)
    detection_mask = np.array(detection_mask)
    ...
    if np.sum(mask_slice_cliped) > 500:
        ...
        np.save(os.path.join(save_gt_detection_path_npy, '{}.npy'.format(gt_file_name)), detection_mask)
        ...
        np.save(os.path.join(save_src_path_no_foreground_npy, '{}.npy'.format(src_file_name+"_no_foreground")), image_data_norm)
        np.save(os.path.join(save_gt_path_no_foreground_npy, '{}.npy'.format(gt_file_name+"_no_foreground")), mask_data_norm)
        np.save(os.path.join(save_gt_detection_path_no_foreground_npy, '{}.npy'.format(gt_file_name + "_no_foreground")))
        with open(os.path.join(save_properties_no_foreground_path_pkl, "{}.pkl".format(src_file_name+"_no_foreground")),
                  'wb') as f:
            pickle.dump(properties, f)
...
if __name__ == '__main__':
    ...
    pkl_path_save = mkdir(os.path.join(path_raw, f'{dataset_type}set_2stage', 'pkl'))
    src_path_save = mkdir(os.path.join(path_raw, f'{dataset_type}set_2stage', 'src'))
    gt_path_save = mkdir(os.path.join(path_raw, f'{dataset_type}set_2stage', 'gt'))
