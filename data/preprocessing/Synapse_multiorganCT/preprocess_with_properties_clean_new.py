...
def read_img(src_file_path, src_file_name, src_save_path, gt_file_path, gt_file_name, gt_save_path,
    ...
    for slice_idx in range(0, mask_data_norm.shape[0]):
        ...
        properties, max_h, min_h, max_w, min_w = get_image_properties(image_slice, mask_slice, classes,
                                                                      class_connection)
        # print(min_h, max_h, min_w, max_w)

        if np.sum(mask_slice_cliped) > 1000:
            ...
            np.save(os.path.join(save_src_path_no_foreground_npy,
                                 '{}_{:03d}.npy'.format(src_file_name + "_no_foreground", slice_idx)), image_slice)
            np.save(os.path.join(save_gt_path_no_foreground_npy,
                                 '{}_{:03d}.npy'.format(gt_file_name + "_no_foreground", slice_idx)), mask_slice)
            with open(os.path.join(save_properties_no_foreground_path_pkl,
                                   "{}_{:03d}.pkl".format(src_file_name + "_no_foreground", slice_idx)),
                      'wb') as f:
                pickle.dump(properties, f)
