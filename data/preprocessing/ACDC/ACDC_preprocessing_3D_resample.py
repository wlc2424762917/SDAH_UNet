...
def read_img(src_file_path, src_file_name, src_save_path, gt_file_path, gt_file_name, gt_save_path, dtype=sitk.sitkFloat32):  # for .mhd .nii .nrrd
    ...
    # print(src_file_name, " data_shape:", image_data_norm.shape)
    zoom_scale_H = 224 / image_data_norm.shape[0]
    zoom_scale_W = 256 / image_data_norm.shape[1]
    patch_gt = zoom(mask_data_norm, zoom=[zoom_scale_H, zoom_scale_W, 1], order=0)
    patch_src = zoom(image_data_norm, zoom=[zoom_scale_H, zoom_scale_W, 1], order=0)
    ...
    # save_path_png = mkdir(os.path.join(save_path, 'png'))
    np.save(os.path.join(save_src_path_npy, '{}.npy'.format(src_file_name)), patch_src)
    np.save(os.path.join(save_gt_path_npy, '{}.npy'.format(gt_file_name)), patch_gt)
...
if __name__ == '__main__':
    ...
    src_path_save = mkdir(os.path.join(path_raw, f'{dataset_type}set_3D_resample', 'src'))
    gt_path_save = mkdir(os.path.join(path_raw, f'{dataset_type}set_3D_resample', 'gt'))
    mask_paths = []
    image_paths = []
    oriImg_paths = []
