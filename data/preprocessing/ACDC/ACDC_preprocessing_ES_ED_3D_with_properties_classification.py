...
def read_img(src_file_path, src_file_name, src_save_path, gt_file_path, gt_file_name, gt_save_path, pkl_save_path, tabular_path, tabular_save_path, classes=[1,2,3]):  # for .mhd .nii .nrrd
    '''
    N*h*W
    :param full_path_filename:
    :return:*H*W
    '''
    group2class = {'NOR': 0, 'MINF': 1, 'DCM': 2, 'HCM': 3, 'RV': 4}
    ...
    with open(tabular_path) as f:
        lines = f.readlines()
        ED_idx = int(lines[0][3:-1])
        ES_idx = int(lines[1][3:-1])
        Group = lines[2][7:-1]
        gt_classification = group2class[Group]
        Height = float(lines[3][7:-1])
        Nb_idx = int(lines[4][8:-1])
        Weight = float(lines[5][7:-1])
    tabular_dict = {'ED_idx': ED_idx, 'ES_idx': ES_idx, 'Group': Group, 'gt_classification': gt_classification, 'Height': Height, 'Nb_idx': Nb_idx, 'Weight': Weight}
    print(tabular_dict)
    ...
    save_tabular_path_json = mkdir(os.path.join(tabular_save_path, src_file_name))
    ...
    with open(os.path.join(save_properties_path_pkl, "{}.pkl".format(src_file_name)), 'wb') as f:
        pickle.dump(properties, f)
    with open(os.path.join(save_tabular_path_json, "{}.json".format(src_file_name)), 'w') as f:
        json.dump(tabular_dict, f)


def read_dataset(src_file_paths, src_save_path, gt_file_paths, gt_save_path, pkl_save_path, tabular_paths, tabular_save_path):
    for idx_data in range(len(src_file_paths)):
        ...
        tabular_path = tabular_paths[idx_data]
        ...
        print(mask_name)
        print(img_name)
        read_img(img_path, img_name, src_save_path, mask_path, mask_name, gt_save_path, pkl_save_path, tabular_path, tabular_save_path)
...
if __name__ == '__main__':
    ...
    ED_src_path_save = mkdir(os.path.join(path_raw, f'ED{dataset_type}set_with_tabular', 'src'))
    ED_gt_path_save = mkdir(os.path.join(path_raw, f'ED{dataset_type}set_with_tabular', 'gt'))
    ED_pkl_path_save = mkdir(os.path.join(path_raw, f'ED{dataset_type}set_with_tabular', 'properties'))
    ED_tabular_save = mkdir(os.path.join(path_raw, f'ED{dataset_type}set_with_tabular', 'tabular'))

    ES_src_path_save = mkdir(os.path.join(path_raw, f'ES{dataset_type}set_with_tabular', 'src'))
    ES_gt_path_save = mkdir(os.path.join(path_raw, f'ES{dataset_type}set_with_tabular', 'gt'))
    ES_pkl_path_save = mkdir(os.path.join(path_raw, f'ES{dataset_type}set_with_tabular', 'properties'))
    ES_tabular_save = mkdir(os.path.join(path_raw, f'ES{dataset_type}set_with_tabular', 'tabular'))
    ...
    for p_id in range(100):
        ...
        ED_mask_paths.extend(glob(os.path.join(path_raw_dataset_type_patient, '*frame04_gt.nii.gz')))
        ...
        ED_image_paths.extend(glob(os.path.join(path_raw_dataset_type_patient, '*frame04.nii.gz')))
        tabular_paths.extend(glob(os.path.join(path_raw_dataset_type_patient, '*cfg')))
    ES_mask_paths.extend(list(set(mask_paths) - set(ED_mask_paths)))
    ES_mask_paths.sort()
    ES_image_paths.extend(list(set(image_paths) - set(ED_image_paths)))
    ES_image_paths.sort()
    tabular_paths.sort()
    print(len(tabular_paths))
    print(len(ED_image_paths))
    print(len(ED_image_paths))
    """for i in range(len(ED_image_paths)):
        print(ED_image_paths[i])"""

    read_dataset(ES_image_paths, ES_src_path_save, ES_mask_paths, ES_gt_path_save, ES_pkl_path_save, tabular_paths, ES_tabular_save)
    read_dataset(ED_image_paths, ED_src_path_save, ED_mask_paths, ED_gt_path_save, ED_pkl_path_save, tabular_paths, ED_tabular_save)
